...
//protected $helper;
class TZT_Input_Widget extends Base_Widget {
/*public function __construct() {
    
	$this->helper = new \Donations\Widgets\Helpers\TZT_Form_Functions(); 
}*/
public function get_name() {
		return 'post_form_input'; 
	}

	public function get_title() {
		return __( 'Post Input', 'client_to_google_sheet' );
	}
    ...
    protected function _register_controls() {
		...
		$repeater->add_control(
			...
			[
				...
				'default' => 'text',
				'options' => [
					'currency'=> __( 'currency', 'client_to_google_sheet' ),
					...
					'hidden_lang'=>__( 'hidden_lang', 'client_to_google_sheet' ),
		 ...
		 $repeater->add_responsive_control(
			...
			[
				...
				'options' => [
					...
					'auto' => __( 'auto', 'client_to_google_sheet' ),
				],
				'default' => 50,
	...
	//	label
			...
			[
				...
				'selectors' => [
					'.rtl {{WRAPPER}} label ' => 'margin-left: {{SIZE}}{{UNIT}};',
					'.rtl {{WRAPPER}} label ' => 'margin-right: 0px;',
					'{{WRAPPER}} label ' => 'margin-right: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} label ' => 'margin-left: 0px;',
				],
			]
		);
		  $this->end_controls_section();
			...
			[
				...
				'default' => '#331060',
				'selectors' => [	'{{WRAPPER}} input , {{WRAPPER}} textarea , {{WRAPPER}} select' => 'color: {{VALUE}}',],
			]
            	);
		  $this->add_control(
			'feilds_bg_color',
			...
			'feilds_margin',
			[
				'label' => __( 'Margin', 'client_to_google_sheet' ),
				'type' => Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px' ],
				'selectors' => [
					'{{WRAPPER}} .form_field' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
               'default' => [
                 'top' => 0,
               'right' => 10,
...
//$this->end_popover();
		...
		$this->end_controls_section();
			...
			'display',
			[
				'label' => __( 'Display Type', 'client_to_google_sheet' ),
				'tab' => \Elementor\Controls_Manager::TAB_STYLE,
			]
		   );
			$this->add_responsive_control(
			...
			[
				...
				'type' => \Elementor\Controls_Manager::SELECT,
				'options' => [
					'flex' => __( 'fit colomn', 'client_to_google_sheet' ),
					'inline' => __( 'inline', 'client_to_google_sheet' ),
					'fullline' => __( 'full line', 'client_to_google_sheet' ),         
						],
				'default' => 'flex',
  ...
  protected function render() {
	  ...
	  $helper = new \Donations\Widgets\Helpers\TZT_Form_Functions($settings,'feilds_row'); 
	 echo $helper->set_inputs();
...
<?php */
    ...
    protected function _content_template(){ 
		//	<# if ( item.filed_type != 'hidden' ) { #>
		?>
<# if ( settings.feilds_row.length ) { #>
	<div class="fild_group  fild_group_inputs {{{ settings.display_type }}} mobile_{{{ settings.display_type_mobile }}} tablet_{{{ settings.display_type_tablet }}}">
   <# _.each( settings.feilds_row , function( item ) { #>
